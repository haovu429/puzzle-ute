...
public interface JobPostRepository extends JpaRepository<JobPost, Long>, JpaSpecificationExecutor<JobPost> {
	...
	@Query("SELECT c FROM Candidate c WHERE c.id in (SELECT a.candidate.id FROM Application a WHERE a.jobPost.id = :jobPostId)")
	...
	@Query("SELECT jp FROM JobPost jp WHERE jp.id in (SELECT a.jobPost.id FROM Application a WHERE a.candidate.id = :candidateId)")
	...
	@Query("SELECT jp FROM JobPost jp, Application ap, Candidate can WHERE can.id = :candidateId AND ap.candidate.id = can.id AND ap.jobPost.id = jp.id")
	...
	@Query("SELECT jp FROM JobPost jp WHERE jp.isActive = :isActive AND jp.createdEmployer.id =:employerId AND jp.isDeleted=FALSE")
	Set<JobPost> findAllByActiveAndCreatedEmployerId(@Param("isActive") boolean isActive,
			@Param("employerId") long employerId);

	@Query("SELECT jp.id FROM JobPost jp WHERE jp.isActive = :isActive AND jp.createdAt >= :createTime AND jp.isDeleted=FALSE")
	List<Long> getJobPostIdByActiveAndLessThenCreatedTime(@Param("isActive") boolean isActive,
	...
	@Query("SELECT jp FROM JobPost jp, JobPostView jpv, User u WHERE jp.id = jpv.jobPostId AND u.id =:userId AND jpv.email = u.email AND jp.isDeleted=FALSE ORDER BY jpv.updatedAt DESC")
	List<JobPost> getRecentViewJobPostByUserId(@Param("userId") long userId);
