...
public interface JobPostRepository extends JpaRepository<JobPostEntity, Long> {
  ...
  @Query(
          "SELECT c FROM CandidateEntity c WHERE c.id in (SELECT a.candidateEntity.id FROM ApplicationEntity a WHERE a.jobPostEntity.id = :jobPostId)")
  Set<CandidateEntity> getCandidateApplyToEmployer(@Param("jobPostId") long jobPostId);
  ...
  @Query("SELECT COUNT(usr) FROM UserEntity usr WHERE usr.id IN (SELECT u.id FROM JobPostEntity jp INNER JOIN jp.viewedUsers u WHERE jp.id =:jobPostId AND u.candidateEntity.id IS NOT NULL AND jp.isDeleted=FALSE)")
